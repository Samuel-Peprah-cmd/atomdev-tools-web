...
export default function RedeemCoupon({ authSession, onSuccess }) {
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-800/40 border border-gray-200 dark:border-gray-700 p-4 sm:p-5 rounded-2xl mt-6">
      ...
      {/* Added flex-col for mobile stacking and sm:flex-row for desktop side-by-side */}
      <form onSubmit={handleRedeem} className="flex flex-col sm:flex-row gap-3 relative">
        <input
          type="text" required value={code} onChange={(e) => setCode(e.target.value.toUpperCase())}
          placeholder="ENTER CODE"
          className="flex-1 w-full px-4 py-3 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-xl text-sm font-semibold outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 uppercase tracking-widest"
        />
        <button type="submit" disabled={loading} className="w-full sm:w-auto px-5 py-3 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-xl transition-colors disabled:opacity-50 flex items-center justify-center sm:min-w-[100px] shadow-sm">
